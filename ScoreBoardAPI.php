...
class name: ScoreBoardAPI
Author: jubayer
Plugin's api: 5.16
For: everybody :)

 */
namespace SkyblockPlugin\khansb;
...
class ScoreBoardAPI{
     ...
     * @return void
     */
    public static function editLineScore(Player $player, int $line, string $replace, string|float $subject): void {
        if (!isset(self::$lineScore[strtolower($player->getName())][$line]) || !self::hasScore($player)){
